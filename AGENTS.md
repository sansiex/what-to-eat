...
底部 Tab：**菜单** → **点餐** → **厨房**（厨房管理、切换厨房）。菜单页（`pages/menu-list/menu-list`）顶部同一行：**「制作新菜单」**（`/images/tab-menu-active.png` 图标）与 **「菜品」**（`/images/tab-dish-active.png` 图标）；**菜品库**从「菜品」进入（`navigateTo` `pages/dish-list/dish-list`）。菜单列表为空且非加载中时展示引导文案：点击「制作新菜单」制作一个菜单吧，有菜单就可以发给大家点餐了。各业务页顶部不再展示厨房选择条，改在厨房管理页通过「切换厨房」操作。
